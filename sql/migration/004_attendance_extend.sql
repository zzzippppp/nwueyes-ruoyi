-- 考勤模块扩展：person_type、locations 门线、behavior_logs 整帧截屏、日汇总表
-- 用法: psql -U postgres -d nwueyes -f ruoyi/sql/migration/004_attendance_extend.sql
-- 前置: 001_core_business.sql（及可选 003）
...
-- ========== behavior_logs：snapshot_url / video，去 source ==========
...
ALTER TABLE behavior_logs DROP COLUMN IF EXISTS face_image_url;
ALTER TABLE behavior_logs DROP COLUMN IF EXISTS body_image_url;
...
INSERT INTO person_daily_attendance (
    stat_date, person_id, location_id, first_enter_at, last_exit_at,
...
FROM (
    SELECT
        ...
        bl.person_id,
        MIN(bl.location_id) AS location_id,
